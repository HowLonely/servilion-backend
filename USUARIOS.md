...
| `demo_ot` | `DIGITADOR_OT` | Demo Digitador OT | Estación 2 de Antofagasta: digitaliza la OT física que llega con la ropa sucia. |
| `demo_empaque` | `DIGITADOR_EMPAQUE` | Demo Digitador Empaque | Estación 3 de Antofagasta: pistolea el morral limpio al empacarlo y resuelve las guías incompletas. |

Dónde entran:

- **Panel web** — `http://localhost:3000/login`
- **Terminal de escritorio** (`servilion-desktop`) — misma cuenta; el menú ofrece
  solo las estaciones que el rol habilita.
...
| Pistolear el empaque | `POST /api/orders/{id}/packing/scan` | `DIGITADOR_EMPAQUE`, `SUPERVISOR` |
...
| Lotes de hotelería | `POST /api/hospitality/...` | `DIGITADOR_OT` / `DIGITADOR_EMPAQUE` / `SUPERVISOR` según la etapa |
